...
void validate_operand(std::string_view op_name, const Tensor& tensor, std::string_view role) {
    TT_FATAL(
        ...
        "{} operation only supports floating-point dtypes (bfloat16, float32, bfloat8_b, bfloat4_b); "
        "{} has dtype {}",
        op_name,
        role,
        tensor.dtype());
...
void BinaryBackwardDeviceOperation::validate_on_program_cache_miss(
    ...
    // read past the end of grad_output or other. logical_shape equality catches
    // the case where TILE padding equalises padded_shape but the tensor still
    // has broadcast semantics (e.g. (1,1,32,128) vs (1,1,1,128) both pad to
    // (1,1,32,128), and the compute path would multiply grad against padding
    // bytes instead of the broadcast operand).
    TT_FATAL(
        grad_output.logical_shape() == input.logical_shape() && other.logical_shape() == input.logical_shape(),
        ...
        other.logical_shape());
    TT_FATAL(
        grad_output.padded_shape() == input.padded_shape() && other.padded_shape() == input.padded_shape(),
    ...
    // The compute kernel packs from DEST to each output CB whose format is fixed to the
    // preallocated buffer's dtype; requiring output_dtype == operand.dtype() per side keeps
    // the pack contract simple and matches the gelu_bw / tanh_bw precedent.
    const auto validate_preallocated =
        [&](const std::optional<Tensor>& preallocated, const Tensor& reference, std::string_view role) {
            ...
            TT_FATAL(
                ...
                role);
            TT_FATAL(
                preallocated->dtype() == reference.dtype(),
