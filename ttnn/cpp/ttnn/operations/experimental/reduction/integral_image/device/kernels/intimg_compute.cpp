...
FORCE_INLINE void cumsum_cube_axis_2(
    ...
    // add_init below reprograms unpack/math per op and every pack has its own reconfig, but
    // nothing else restores the source formats after the previous block's helpers.
...
void kernel_main() {
    ...
    // The kernel's single hardware boot. cumsum_cube_axis_2 is the first helper to touch the
    // Tensix engines, so it is booted with that helper's operands.
