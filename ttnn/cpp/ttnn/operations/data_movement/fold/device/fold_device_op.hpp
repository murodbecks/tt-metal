...
// SRC0 + SRC1 depth per C-tile. 2 = double-buffered so untilize (compute→SRC1) and gather
// (writer←SRC1) pipeline through a super-block; the writer now barriers+pop_fronts inside the
// w_tile loop, so single-buffer strictly serialized the two. Factory + capacity predicate both
// scale by this so any depth change stays in sync.
