...
tt::tt_metal::ProgramDescriptor SparseMatmulMultiCoreReuseMcast1DProgramFactory::create_descriptor(
    ...
    // Mcast args
    // Semaphore ids are hand-assigned on the descriptor path -- there is no CreateSemaphore to
    // allocate them. 0 and 1 are exactly what CreateSemaphore returned on a fresh program, so the
    // ids baked into the sender/receiver compile-time args below (and hence the kernel ELFs) are
    // unchanged. The descriptors themselves are pushed alongside the CBs further down.
    ...
    // unpack_to_dest_mode is deliberately left empty: the legacy ComputeConfig default was also
    // empty, so this preserves behaviour exactly. opt_level stays unset for the same reason -- the
    // descriptor path applies O2 for data movement and O3 for compute, matching the legacy defaults.
    ...
    // program structurally identical. None of these CBs is tensor-backed or globally allocated --
    // in0/in1/sparsity/output are all interleaved here -- so no CBDescriptor carries a .buffer or
    // .tensor and there is nothing for the framework to patch on the CB side.
    ...
    for (uint32_t i = 0; i < num_cores; ++i) {
        ...
        // in0 receiver and in 1 sender
            ...
            // No buffer addresses in the receiver's args, so no bindings are needed.
