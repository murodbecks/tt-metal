...
TEST_F(LLKQuasarMeshDeviceSingleCardFixture, QuasarCbL1ReadApi) {
    ...
    // Int32 rather than UInt32: Quasar's supported-format list (is_supported_quasar) has no UInt32,
    // and Metal 2.0 validates the DFB's data format against the arch where the legacy
    // dfb::CreateDataflowBuffer path did not. Both are 4-byte datums, so the tile size and the ring
    // arithmetic below are unchanged, and this test only ever moves raw bit patterns through.
